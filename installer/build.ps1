...
Copy-Item -Force $PathToExecutable Work/WinAgent.exe

Write-Verbose "Creating WinAgent-${Version}-${Arch}.msi"
$wixArch = @{"amd64" = "x64"; "386" = "x86"}[$Arch]
$wixOpts = "-ext WixFirewallExtension -ext WixUtilExtension"
Invoke-Expression "WiX\candle.exe -nologo -arch $wixArch $wixOpts -out Work\WinAgent.wixobj -dVersion=`"$Version`" WinAgent.wxs"
Invoke-Expression "WiX\light.exe -nologo -spdb $wixOpts -out `"Output\WinAgent-${Version}-${Arch}.msi`" Work\WinAgent.wixobj"
